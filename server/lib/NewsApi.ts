 ...
 * @returns NewsElement[]
 */
async function ostfaliaNewsRequest(
  newsSelector: string,
): Promise<NewsElement[]> {
  ...
  query.append(
    ...
    'fq=type:of-news' + // not stricly neccessary, but kept here just in case
      '&fq=parent-folders:"/sites/default/de/' +
      newsSelector +
      '/.content/"' + // .content could be removed as well
      '&sort=newsdate_de_dt desc' + // order the articles by date in the SOLR requests
      '&rows=25',
  ); // limit the number of requested rows, otherwise server error
...
async function campus38NewsRequest(): Promise<NewsElement[]> {
  const response = await fetch('https://www.campus38.de/newsfeed.xml').then(
    (res) => res.text(),
  );
  ...
  return $('entry')
    .map(function () {
      return <NewsElement>{
        ...
        date: moment($('published', this).text().trim().split('T')[0])
          .utcOffset('+0100')
          .toDate(),
      };
    })
...
function truncateAndSortNews(
  ...
  news = news.filter(
    (article) => moment(article.date).diff(moment(), 'days') < 3,
  );
  // Filter any duplicate entries (based on title+desc)
  news = news.filter(
    (v, i, a) =>
      a.findIndex((t) => t.title === v.title && t.text === v.text) === i,
  );
  ...
  news = news.map((article) => {
    ...
    if (text.length > MAX_LEN) {
      ...
      if (lastDotIndex > MAX_LEN / 2)
        text = text.substr(0, lastDotIndex + 1); // cut text at end of sentence
...
export default async function getNews(
  ...
  const allowedSelectors = facultySelectors.concat(
    campusSelectors,
    otherSelectors,
  );
    ...
    newsSelectors.map(async (newsSelector) => {
      ...
      try {
        ...
        return (await cache.wrap(
          key,
          async () => {
            let newsEls: NewsElement[] =
              newsSelector !== 'campus38'
                ? await ostfaliaNewsRequest(newsSelector)
                : await campus38NewsRequest();
            newsEls = newsEls.map((article) => ({
              ...article,
              source: newsSelector,
            }));
            return newsEls;
          ...
          { ttl: CACHE_SECONDS },
        )) as Promise<NewsElement[]>;
      } catch (e) {
        if (!(e instanceof Error)) throw e;
        console.log(
          `Error while loading news for "${newsSelector}": ${e.message}`,
        );
        return [];
      }
    }),
