<template>
  <v-bottom-sheet v-model="open" inset>
    <v-list>
      ...
      <v-list-item :href="downloadLink" @click="open = false">
        <v-list-item-avatar>
      ...
      <v-list-item v-clipboard="httpLink" @click="open = false">
        <v-list-item-avatar>
          <v-avatar size="24px" tile>
      ...
      <v-list-item :href="webcalLink" @click="open = false">
        <v-list-item-avatar>
          <v-avatar size="24px" tile>
...
export default {
  ...
  computed: {
    calendarPath() {
      ...
      return (
        endpoint + this.timetableIds.join(',') + '/' + this.titleIds.join(',')
      );
    },
    webcalLink() {
