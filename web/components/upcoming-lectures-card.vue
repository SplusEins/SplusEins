<template>
  <v-card class="fill-height">
    <v-card-title>
      ...
      <v-btn v-show="hasSubscribableTimetables" icon @click="dialogOpen = true">
        <v-icon size="32px">
          {{ mdiMenuDown }}
        </v-icon>
      </v-btn>
    ...
    <v-card-text v-if="nextEvent !== undefined" class="text--primary">
      <b>{{ nextEvent.title }} {{ nextEvent.lecturer }}</b>
      <br />
      <span class="text--secondary">Datum:</span>
      {{ nextEvent.start.format('dddd, DD.MM.YYYY') }}
      <br />
      <span class="text--secondary">Uhrzeit:</span>
      {{ nextEvent.start.hour() }}:{{
        nextEvent.start.minute() === 0 ? '00' : nextEvent.start.minute()
      }}
      Uhr
      <br />
      <span v-if="!!nextEvent.room">
        <span class="text--secondary">Raum:</span>
        <span v-html="nextEvent.room" />
      </span>
    </v-card-text>
    <v-card-text
      v-else-if="hasSubscribableTimetables && nextEvent === undefined"
    >
      <i>Keine weiteren Vorlesungen in dieser Woche!</i>
    </v-card-text>
    <v-card-text v-else>
      <i
        >Markiere einen Stundenplan als Favoriten, um ihn hier anzuzeigen.
        Alternativ kannst du auch einen personalisierten Plan erstellen.</i
      >
    </v-card-text>
...
export default {
  ...
  computed: {
    selectedItem: {
      get() {
        return this.subscribedTimetable;
      },
      set(value) {
        this.setSubscribedTimetable(value);
      },
    },
    ...mapState({
      upcomingEvents: (state) => state.splus.upcomingEvents,
      upcomingLecturesTimetable: (state) =>
        state.splus.upcomingLecturesTimetable,
