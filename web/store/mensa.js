...
export const getters = {
  getNextAvailablePlan: (state) => {
    ...
    const isOld = dayjs().day() != 5 && dayjs().isSame(state.plans[0].dayPlan[0].date, 'day') && dayjs().hour() > 14
...
export const actions = {
  async load ({ state, commit }) {
    if (state.plans != null && state.plans.length > 0 && dayjs().isSame(state.plans[0].dayPlan[0].date, 'day')) {
